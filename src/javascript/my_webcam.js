...
function picture_to_url(element) {
    ...
    fileReader.onload = function(fileLoadedEvent) {
      ...
      if (postData.length > 0) {
        if (srcData.match(/png/)) {
          ...
          xhr.open("POST", "../my_php/pic_storage.php", true);
...
(function() {
  ...
  startbutton.addEventListener(
    "click",
    function(ev) {
      if (postData.length > 0) picture_snap();
      else {
        var div = document.createElement("div");
        var p = document.createElement("p");
        var text2 = document.createTextNode("Please choose a filter");
